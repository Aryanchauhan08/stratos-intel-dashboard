...
def prune_processed_activity(db) -> None:
    ...
    If the table contains more than STORAGE_LIMIT rows, the oldest records
    (by processed_at ASC) are deleted until exactly STORAGE_LIMIT rows remain.
    Uses a single DELETE … WHERE id NOT IN (…) statement so that no Python-level
    loop is needed and the operation is atomic within the caller's transaction.
...
# Core worker logic
# ---------------------------------------------------------------------------
def run_worker(limit: int = 1000, batch_size: int = 50, poll_interval: int = 10) -> None:
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=1000,
        metavar="N",
        help="Maximum number of rows to process in this run (default: 1000).",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
